...
class MaterialThemeDocProvider : DocumentationProvider {
    ...
    override fun generateDoc(element: PsiElement?, originalElement: PsiElement?): String? {
        ...
        return when (match.kind) {
            KtMatchers.Match.Kind.COLOR -> idx.getColor(match.name)
                ?.let { "<b>Material color:</b> ${match.name}<br/><code>$it</code>" }
            ...
            KtMatchers.Match.Kind.SHAPE -> idx.getShape(match.name)?.let { "<b>Shape:</b> ${match.name}<br/>$it" }
        }
    }
}