...
class ThemeIndexService(private val project: Project) : DumbAware {
    ...
    private val executor = AppExecutorUtil.createBoundedScheduledExecutorService("ThemeIndexService", 1)
    ...
    private fun rebuildSafe() {
        ReadAction.nonBlocking<Unit> {
                        ...
                        when (expression.calleeExpression?.text) {
                            ...
                            "Typography" -> {
                                expression.valueArguments.forEach { arg ->
                                    ...
                                    fun readSp(k: String): Float? =
                                        Regex("$k\\s*=\\s*(\\d+(?:\\.\\d+)?)\\.sp").find(body)?.groupValues?.get(1)?.toFloat()
                            ...
                            "Shapes" -> {
                                expression.valueArguments.forEach { arg ->
                                    val key = arg.getArgumentName()?.asName?.asString() ?: return@forEach
                                    val body = arg.getArgumentExpression()?.text ?: return@forEach
                                    val corner = Regex("RoundedCornerShape\\(([^)]+)\\)")
            ...
            if (newColors.isEmpty())
                log.warn("[ThemeIndexService] No theme colors resolved. Check that darkColorScheme/lightColorScheme exist and color args reference either Color(0x...) or top-level vals initialized with Color(0x...).")
        }
            .coalesceBy(this)
            .inSmartMode(project)
    ...
    private fun parseColorLiteral(text: String?): String? {
        ...
        Regex("""(?:^|[\s(])(?:androidx\.compose\.ui\.graphics\.)?Color\s*\(\s*0x([0-9A-Fa-f]{6,8})\s*\)""")
            .find(text)?.groupValues?.get(1)?.let { hex ->
                return "#" + hex.takeLast(6).uppercase()
            }
        return null
    }
}